...
Same as the origional yolov3 repo. Please follow the instructions of the origional readme below.

**Dataset**

Using dataset from https://gitlab.com/EnginCZ/yolo-with-distance .

The distance information in this dataset is not accurate enough, which can be inferred from the first image above, named 006740.png. The ground truth distance of the cars from opposite lane are both 32m, which is inconsistent with our knowledge.

However, this is the only dataset I can find with distance information.
...
```
python train.py --data kitti.yaml --epochs 100 --cfg yolov5n.yaml  --batch-size 16
```

Output of training process at runs/train/exp, including weights, PR_curve, etc.
...
Output images at runs/detect/exp.
