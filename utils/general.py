...
def non_max_suppression(
    ...
    nc = 7 #prediction.shape[2] - nm - 5  # number of classes
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #mask = x[:, mi+1:]  # zero columns if no masks
        # Detections matrix nx6 (xyxy, conf, cls)
        if multi_label:
            i, j = (x[:, 5:-1] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, 5 + j, None], j[:, None].float(),x[i,-1].reshape(i.shape[0],1)), 1) #mask[i]), 1)
        else:  # best class only
            conf, j = x[:, 5:mi].max(1, keepdim=True)
            
            x = torch.cat((box, conf, j.float(),x[:,-1].reshape(x.shape[0],1)), 1)[conf.view(-1) > conf_thres]# mask), 1)[conf.view(-1) > conf_thres]
        # Filter by class
