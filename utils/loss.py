...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        ldis = torch.zeros(1, device=self.device)  # distance loss
        
        tcls, tbox, indices, anchors, tdis = self.build_targets(p, targets)  # targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                pxy, pwh, _, pcls, pdis = pi[b, a, gj, gi].split((2, 2, 1, self.nc, 1), 1)  # target-subset of predictions
                ...
                # Distance
                #print("predicted distance", pdis.shape,pdis.dtype)
                #print("target distance ",tdis[i].shape,tdis[i].dtype)
                ldis += self.MSEloss(pdis,tdis[i])
        ...
        return (lbox + lobj + lcls + ldis) * bs, torch.cat((lbox, lobj, lcls, ldis)).detach()
    ...
    def build_targets(self, p, targets):
        ...
        """
        na, nt = self.na, targets.shape[0]  # number of anchors, targets: num_targets x 6+1
        
        tcls, tbox, indices, anch ,tdis= [], [], [], [], []
        #print(targets)
        gain = torch.ones(8, device=self.device)  # normalized to gridspace gain
        ai = torch.arange(na, device=self.device).float().view(na, 1).repeat(1, nt)  # same as .repeat_interleave(nt)    na x nt
        targets = torch.cat((targets.repeat(na, 1, 1), ai[..., None]), 2)  # append anchor indices
        #                   num_targets*na x 7+1       na x num_targets x 1 , anchor indices      num_targets*na x 7+1
        g = 0.5  # bias
        off = (
            torch.tensor(
                [
        ...
        for i in range(self.nl):   #itertate over layers
            anchors, shape = self.anchors[i], p[i].shape
            gain[2:6] = torch.tensor(shape)[[3, 2, 3, 2]]  # xyxy gain

            # Match targets to anchors
            t = targets * gain  # shape(3,n,7)  batch=1 (3,16,7)
            ...
            tcls.append(c)  # class
            true_distance = t[:,6].reshape(t.shape[0],1)
            scale = torch.full((t.shape[0],1),150,device=self.device)
            tdis.append(true_distance/scale)

            #print("tdis",tdis.shape)

        return tcls, tbox, indices, anch, tdis
